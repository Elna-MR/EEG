...
def select_10_20_channels(raw, num_channels=19):
    """
    Select only standard 10-20 system EEG channels.
...
def main():
    ...
    ap.add_argument("--root", default="../data", help="Folder containing sub-*/eeg/*.vhdr files (default: ../data)")
    ap.add_argument("--out", default="../packed/cpcgx_pain.npz", help="Output NPZ (default: ../packed/cpcgx_pain.npz)")
    args = ap.parse_args()
    ...
    for f in files:
        ...
        # Select only standard 10-20 system channels (19 channels) BEFORE filtering
        # This matches ds005284 approach for consistency
        raw = select_10_20_channels(raw, num_channels=19)
        print(f"[INFO] {sub_id}: Using {len(raw.ch_names)} standard 10-20 channels")
        
        # After channel selection, all remaining channels are EEG
        picks_eeg = mne.pick_types(raw.info, eeg=True, exclude=[])
        if len(picks_eeg) == 0:
            print(f"[WARN] {sub_id}: No EEG channels after selection, skipping")
            continue
